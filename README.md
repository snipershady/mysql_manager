...
```
target/mysql-manager-1.0.6.jar
```
...
```bash
java -jar target/mysql-manager-1.0.6.jar
```
...
```bash
nohup java -jar target/mysql-manager-1.0.6.jar \
  --spring.datasource.password=SceglieUnaPasswordSicura! \
  --mysql.manager.password=TuaPasswordRoot \
...
```bash
mvn clean package -DskipTests
sudo cp target/mysql-manager-1.0.6.jar /opt/mysql-manager/mysql-manager.jar
sudo systemctl restart mysql-manager
```
